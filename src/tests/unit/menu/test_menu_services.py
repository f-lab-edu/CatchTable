...
from app.service_layer.services import login, restaurant, menu
...
def test_success_add_menu(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu):
...
def test_fails_add_menu_when_menu_existed(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu):
    add_menu(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu)
    with pytest.raises(errors.DuplicatedException) as error:
        menu.add_menu(1, valid_menu, override_get_uow)
    assert str(error.value) == "existed data"

def test_success_get_menu_for_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu):
    add_menu(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu)
    result = menu.get_menu_for_restaurant(1, override_get_uow)
    assert result == valid_menu

def test_fails_get_menu_for_restaurant_when_menu_not_existed(override_get_uow, valid_owner_with_password, valid_restaurant):
...
def test_success_update_menu(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu):
    add_menu(override_get_uow, valid_owner_with_password, valid_restaurant, valid_menu)
