...
from app.service_layer.services import registration, login
...
def test_success_authenticate_user(override_get_uow, valid_owner_with_password):
    ...
    result = login.authenticate_user(
        valid_owner_with_password.email,
...
def test_fail_authenticate_user_when_password_not_matched(
    override_get_uow, valid_owner_with_password
):
    registration.add_user(valid_owner_with_password, override_get_uow)
    with pytest.raises(errors.PasswordNotMatchedException) as error:
