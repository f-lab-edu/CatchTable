...
from app.service_layer.services import login, restaurant
...
def test_success_add_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant):
...
def test_fails_add_restaurant_when_input_invalid_data(override_get_uow, valid_owner_with_password, invalid_restaurant):
    login.add_user(valid_owner_with_password, override_get_uow)
...
def test_fails_add_restaurant_when_invalid_owner_id(override_get_uow, valid_owner_with_password, valid_restaurant):
    login.add_user(valid_owner_with_password, override_get_uow)
...
def test_fails_add_restaurant_when_data_already_existed(override_get_uow, valid_owner_with_password, valid_restaurant):
    add_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant)
    with pytest.raises(errors.DuplicatedException) as error:
        restaurant.add_restaurant(1, valid_restaurant, override_get_uow)
    assert str(error.value) == "existed data"

def test_success_get_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant):
    login.add_user(valid_owner_with_password, override_get_uow)
    restaurant.add_restaurant(1, valid_restaurant, override_get_uow)
...
def test_success_get_restaurant_list(override_get_uow, valid_owner_with_password, valid_restaurant):
    add_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant)
    result = restaurant.get_restaurant_list("city", "seoul", override_get_uow)
    assert result == [valid_restaurant]

def test_fails_get_restaurant_list_when_input_invalid_filter(override_get_uow, valid_owner_with_password, valid_restaurant):
    add_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant)
    with pytest.raises(errors.NotFoundException) as error:
        restaurant.get_restaurant_list("something", "something", override_get_uow)
    assert str(error.value) == "filter not existed"

def test_success_update_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant):
    add_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant)
...
def test_success_delete_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant):
    add_restaurant(override_get_uow, valid_owner_with_password, valid_restaurant)
